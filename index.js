...
app.use(
	...
	function (req, res, next) {
		...
		if (
			['/game/', '/game/index.html'].includes(req.originalUrl)
		) {
...
function getRoomIDFromSocketCookie(socket_obj) {
	let result = null;
	try {
		const parsed_cookie = cookie.parse(
			socket_obj.request.headers.cookie,
		);
...
function linkSocketToRoom(socket_obj, next) {
	const room_id = getRoomIDFromSocketCookie(socket_obj);
...
io.on('connection', socket => {
	...
	socket.emit(
		'player_number',
		room.getPlayerNumberFromSocketID(socket.id),
	);
	io.to(room.id).emit('player_list', room.listOfPlayerSockets);

	// check room after player disconnect
	socket.on('disconnect', () => {
		const playerNumberInRoom = room.getPlayerNumberFromSocketID(
			socket.id,
		);
